...
export async function getSeries(urlOrPath: string): Promise<MangaUpdatesSeriesDetails> {
  ...
  const title =
    ...
    normalizeWhitespace($("title").first().text().replace(/\s*-\s*MangaUpdates\s*$/, ""));
  ...
  return {
    ...
    latestReleases: cleanList(rawSections["Latest Release(s)"] ?? [], [/^Search for all releases/i, /^19 years ago$/i]),
    forum: {
    ...
    activityStats: cleanList(rawSections["Activity Stats"] ?? [], [/^(Weekly|Monthly|3 Month|6 Month|Year)$/]),
...
async function main() {
  ...
  if (!command) {
    console.error("Usage: bun run sources/manganime.ts <search|getSeries> <query-or-url>");
    process.exit(1);
  }
  ...
  if (command === "search") {
    ...
    if (!query) {
      console.error("Usage: bun run sources/manganime.ts search <query>");
      process.exit(1);
    }
  ...
  if (command === "getSeries") {
    ...
    if (!url) {
      console.error("Usage: bun run sources/manganime.ts getSeries <url>");
      process.exit(1);
    }
