...
function extractCredits($: cheerio.CheerioAPI): IssueCredit[] {
  const authorRow = $("#cuerpo2_ficha .row-fluid").filter((_, element) => {
...
function extractTitleAndSubtitle($: cheerio.CheerioAPI) {
  ...
  const subtitle = nestedSubtitle || normalizeWhitespace($("#titulo_ficha .subtitulo").first().text()) || null;
...
async function scrapeIssue(url: string, context: TebeosferaOcrContext): Promise<ScrapedIssue> {
  ...
  const collectionLink = $("#cuerpo2_ficha > .row-fluid.dato").first().find('a[href*="/colecciones/"]').first();
  ...
  const promotionalText = cleanPromotionalText(normalizeWhitespace($(".div_recension").first().text()) || null);
  const extraNotes = normalizeWhitespace($(".div_notas_numeros").first().text()) || null;
  const originTitle = cleanOriginTitle(normalizeWhitespace(
    $("#cuerpo2_ficha .row-fluid")
      .filter((_, element) => normalizeWhitespace($(element).find(".etiqueta").first().text()).startsWith("Origen"))
      .first()
      .find("em")
      .first()
      .text()
...
export async function scrapeComicMetaFromOcrContext(
  ...
  const searchTitle = options.searchTitle?.trim() || context.title?.trim() || context.collection?.trim() || "";
  if (!searchTitle) {
